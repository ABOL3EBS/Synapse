...
- **Canonical-vs-directional migration (2026-08-18):** Steps A–F complete. `ResolvedFlow { local_ip, local_port, remote_ip, remote_port }` stored on `FlowRecord`; `FlowFeatures::from_flow()` requires `&ResolvedFlow` (compile error without it); all detectors use `resolved.*` for port/IP access; `VerdictDecided` carries `remote_ip: IpAddr`; schema V5 + dashboard reads it verbatim. 189 workspace tests, 0 failures.
