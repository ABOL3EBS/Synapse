...
- **Dashboard (Tauri + React):** 4 screens built and verified in real Tauri window (2026-08-01). Protection (shield + active blocks), Activity (live feed, verdict badges, stagger animation), Report/Stats (sparkline, detector breakdown, top apps, animated counters), Threat Map (react-globe.gl, pulsing red rings per threat country, auto-rotate, ranked sidebar). All backed by real SQLite via Tauri IPC commands in `src-tauri/src/lib.rs`. Globe texture fix: `earth-day.jpg` copied to `public/` and used as local path — protocol-relative `//unpkg.com/...` fails in Tauri webview (`tauri://unpkg.com/...` is invalid). Detector breakdown: `AND score > 0` added — was showing all 6 detectors at identical 10,922 count. Gateway fix: `rt_buf_find_gateway` now prefers `dst=0.0.0.0` route over VPN split-tunnel routes. UI reflow (2026-08-07): window resizable (900×620 min), Protection/Report centering (max-w-3xl/max-w-4xl), Globe ResizeObserver fills container, busy_timeout=5000 on both SQLite connections. CrossFlow per-PID keying (2026-08-07): `ip_stats` rekeyed to `(u32, IpAddr)` — browser tabs no longer aggregate. FlowBehavior: UDP/443 false-positive removed (QUIC/HTTP3). Gateway detection hardening (2026-08-07): RT_DUMP fallback when no RTF_GATEWAY routes, last_known_gw caching in refresh thread (see rule 16).
