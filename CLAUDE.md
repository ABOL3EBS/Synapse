...
- **Dashboard (Tauri + React):** 5 screens built and verified in real Tauri window. Protection (shield + active blocks), Activity (live feed, verdict badges, stagger animation), Report/Stats (sparkline, detector breakdown, top apps, animated counters), Threat Map (react-globe.gl, pulsing red rings per threat country, auto-rotate, ranked sidebar), Settings (active blocks with live countdown + reason subtitle, detection thresholds, CrossFlow exclusions, unblock button). All backed by real SQLite via Tauri IPC commands in `src-tauri/src/lib.rs`. UI polish (2026-08-12): Block dot/badge red (was green), composite score in technical-details panel, BlockRow reason subtitle (strips boilerplate prefix), live countdown via `useNow` hook, live relative timestamps, Activity polls every 5 s / Protection every 10 s, new-item green flash (`card-flash`) on Activity feed refresh. **Direction migration (2026-08-18):** `remote_ip_text` column (schema V5) now persisted at verdict time from `resolved.remote_ip`; dashboard reads it directly — no re-derivation from canonical a/b ordering.
- **Canonical-vs-directional migration (2026-08-18):** Steps A–F complete. `ResolvedFlow { local_ip, local_port, remote_ip, remote_port }` stored on `FlowRecord`; `FlowFeatures::from_flow()` requires `&ResolvedFlow` (compile error without it); all detectors use `resolved.*` for port/IP access; `VerdictDecided` carries `remote_ip: IpAddr`; schema V5 + dashboard reads it verbatim. **Schema V6 (2026-09-13):** `enforcement_log.requested`/`confirmed` dropped, spool payload_version replay gate, DB-level CHECK constraints — verified byte-identical SQL against a live production-DB copy (all 223/898/5388 rows preserved, 0 FK violations, 0 orphaned findings; verdicts.id copied verbatim — first formulation orphanned every finding, caught in the live demo). 229 workspace tests (+12 Tauri), 0 failures.
