...
| Agent binary | `crates/agent/src/main.rs` | 279 | Startup + orchestration only. BPF fd receive, IPC reader thread, local IP detection, CaptureEngine creation, capture loop delegation. Tests for BPF wordalign, packet parsing. |
| Capture engine | `crates/agent/src/capture.rs` | 915 | `CaptureEngine` struct: BPF read buffer, packet parsing (IPv4+IPv6), flow tracker integration, enrichment dispatch, verdict handling via `handle_verdict()`, active-flow re-evaluation. Direction helpers (`determine_local_port`, `determine_remote_ip`, `detect_local_ip`). BpfHdr struct. 16 tests. |
...
**Total:** 5,977 lines across 12 files.
