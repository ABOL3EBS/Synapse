...
6. ~~DnsAnalyzer blocklist uses substring matching, not label-aware~~ ✅ — `score_blocklist()` now uses exact-or-subdomain matching for full-domain entries and `has_label()` for single-label entries. `pub(crate) fn has_label()` is a single shared implementation (no independent copy that could drift). Same fix class as the original RuleDetector `ec2`/`c2` false-positive; the same `has_label()` logic is now available to both. 7 regression tests cover: substring non-match, exact match, subdomain match, `ec2` vs `c2` non-match, standalone `c2` match, and direct `has_label()` cases.
