...
│   │       └── types.rs          # 898 lines — ValidatedBlock, BlockId, DesiredFirewallState,
...
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── main.rs           # 346 lines — engine pipeline, worker threads, capture loop
...
│   │       ├── enrichment/       # 718 lines — async worker pool (DNS, process attribution,
...
│   │       └── storage/          # mod.rs (StorageWorker + 3-variant StorageEvent), schema.rs (V2 DDL,
│                             #   PRAGMA user_version migration — V1: base tables; V2: metadata table
│                             #   for wall-clock retention persistence), spool.rs (CriticalSpool —
│                             #   separate DB with synchronous=FULL for crash-safe enforcement log),
