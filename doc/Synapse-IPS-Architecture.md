...
│   │       └── storage/          # mod.rs (1451 lines — StorageWorker + 3-variant StorageEvent;
│                             #   VerdictDecided carries remote_ip: IpAddr persisted as
│                             #   remote_ip_text in schema V5), schema.rs (606 lines — idempotent
│                             #   PRAGMA user_version migrations V1–V6; V6 drops
│                             #   enforcement_log.requested/confirmed, adds CHECK constraints,
│                             #   preserves verdicts.id so findings FKs survive),
│                             #   spool.rs (CriticalSpool — separate DB with
│                             #   synchronous=FULL for crash-safe enforcement log; payload_version
│                             #   column gates replay), models.rs (row structs, serde::Serialize),
│                             #   reader.rs (StorageReader, read-only connection, query_only=ON)
│   │
...
- **v12 (schema V6 — dead column drop, spool payload versioning, DB-level CHECKs; 2026-09-13):** Storage hardening, verified against a live copy of the production DB (229 workspace + 12 Tauri tests). (1) `enforcement_log.requested`/`confirmed` dropped via a rename-dance migration that recreates all three tables (`PRAGMA foreign_keys=OFF` → `BEGIN` → `*_v6` → copy → `DROP` → rename → recreate 9 indexes → `COMMIT` → `foreign_key_check` → `ON`). The verdicts copy **preserves `id` verbatim** — production `verdicts.id` lives in the 200k range (V4 dedup keeps `MAX(id)`; retention deletes old rows) and `detector_findings.verdict_id` references it; the first formulation omitted the id and orphanned all 5388 findings, caught live by `foreign_key_check` before shipping. (2) Spool entries carry `payload_version` (const `PAYLOAD_VERSION = 1` in `spool.rs`; pre-V6 spools get the column via `ALTER TABLE ... DEFAULT 1`); replay refuses unrecognised versions — `warn!` + entry stays unconfirmed, never parsed/dropped. (3) CHECK constraints at the DB level: `action IN ('Block','Unblock')`, `verdict IN ('Alert','Block')`, `score`/`confidence`/`composite_score` ∈ [0,1]. Live-verified on the migrated copy: `Ban` action rejected by CHECK, 0 FK violations, 0 orphans, all 223/898/5388 rows preserved — the early buggy run's violation storm was 100 % the `id`-drop defect (no pre-existing orphans existed; see `doc/DB-Architecture.md` §3.2). `src-tauri` queries updated (`blocked_addresses` drops `requested=1`, semantically identical; `write_unblock_to_db` drops the columns). §5 storage/ annotation updated.
- **v11 (dashboard real app icons + flag images; 2026-09-01):** Dashboard — Activity feed and Report > Flagged Apps now render the real macOS application icon per verdict, resolved at runtime via a new `get_app_icon` Tauri command using `NSWorkspace` (no static icon map). `find_parent_app_bundle()` in `src-tauri/src/lib.rs` walks `process_path` upward and returns the **outermost** `.app` (nested Electron/Chromium helper bundles like `Brave Browser Helper.app` carry no icon resource; the main app owns it). Icon extracted as `NSImage → TIFF → NSBitmapImageRep → PNG`, returned as base64, cached in `IconCache` keyed by the resolved main `.app` bundle so all helper variants share one NSWorkspace call. Two bugs fixed during implementation: (1) EXC_BREAKPOINT crash — `stringWithString:` called with a raw `&str` where an `NSString*` was expected (fixed to `stringWithUTF8String:`); (2) generic/blank icons — `&str::as_ptr()` is not null-terminated so `stringWithUTF8String:` read garbage (fixed by wrapping the path in `CString`); plus the innermost→outermost bundle fix. `ActivityItem` and `TopApp` both gained `process_path: Option<String>` (already selected by the queries). Frontend: new reusable `src/components/AppIcon.tsx` (real `<img>` on success, coloured-letter fallback for system processes like `rapportd`, module-level per-path promise cache, `appHue`/`appInitial` exported); both Activity rows and Flagged Apps render at 40×40px via the `size` prop with no second icon implementation. Flag emojis replaced with real images via `flag-icons@7.5.0` sprite (`src/components/FlagIcon.tsx`) in Top Threat Sources and the Threat Map sidebar. Sidebar note: `doc/STATUS.md` also records unrelated prior-session agent refactor (extracted `VerdictContext` in `capture/mod.rs`; trivial `settings.rs` reformat).
